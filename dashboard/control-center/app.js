...
function bindEvents() {
  ...
  if (execReportBtn) {
    execReportBtn.addEventListener("click", async () => {
      state.execReportLoading = true;
      render();
      try {
        const res = await fetch("/api/agent/executive-report");
        if (res.ok) {
          state.execReportResult = await res.json();
          state.syncMessage = "Executive report generated.";
          state.syncLevel = "connected";
        }
      } catch { /* demo ok */ }
      state.execReportLoading = false;
      render();
    });
  }
  ...
  const closeConnectSources = document.getElementById("connect-sources-close");
  const cancelConnectSources = document.getElementById("connect-sources-cancel");
  [closeConnectSources, cancelConnectSources].forEach((btn) => {
    if (btn) {
      btn.addEventListener("click", () => {
  ...
  const connectSourcesOverlay = document.getElementById("connect-sources-overlay");
  if (connectSourcesOverlay) {
    connectSourcesOverlay.addEventListener("click", (e) => {
      if (e.target === connectSourcesOverlay) {
        state.connectSourcesOpen = false;
        render();
      }
    });
  ...
  if (saveConnectSources) {
    saveConnectSources.addEventListener("click", async () => {
      const supabaseUrl = document.getElementById("cs-supabase-url")?.value.trim() || "";
      const supabaseKey = document.getElementById("cs-supabase-key")?.value.trim() || "";
      const shopifyDomain = document.getElementById("cs-shopify-domain")?.value.trim() || "";
      const shopifyToken = document.getElementById("cs-shopify-token")?.value.trim() || "";

      if (!supabaseUrl || !supabaseKey) {
        state.connectSourcesError = "Supabase URL and Anon Key are required.";
        render();
        return;
      }

      state.connectSourcesSaving = true;
      state.connectSourcesError = null;
      render();

      try {
        localStorage.setItem("evics_supabase_url", supabaseUrl);
        localStorage.setItem("evics_supabase_anon_key", supabaseKey);
        if (shopifyDomain) localStorage.setItem("evics_shopify_domain", shopifyDomain);
        if (shopifyToken) localStorage.setItem("evics_shopify_token", shopifyToken);

        state.connectSourcesFields = { supabaseUrl, supabaseAnonKey: supabaseKey, shopifyDomain, shopifyToken };

        // Re-initialize Supabase client with new credentials
        if (window.iagtSupabase && typeof window.iagtSupabase.reinitialize === "function") {
          window.iagtSupabase.reinitialize(supabaseUrl, supabaseKey);
        }

        state.connectSourcesSaved = true;
        state.connectSourcesSaving = false;
        render();

        // Re-hydrate after short delay
        setTimeout(async () => {
