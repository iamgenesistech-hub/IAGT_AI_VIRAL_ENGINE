...
render = function () {
  ...
  try {
    ...
    if (app && typeof result === "string" && result.length > 0) {
      ...
      // Explicitly hide the boot splash once content is in the DOM
      const splash = document.getElementById("evics-boot-splash");
      if (splash) splash.style.display = "none";
    }
  } catch (err) {
    console.error("[EVICS] Render error:", err);
  }
  try { bindEvents(); } catch (e) { console.error("[EVICS] bindEvents error:", e); }
