...
// Canonical checkout host: the Shopify-hosted store where product URLs actually resolve
// to a working add-to-cart / checkout flow. Squarespace mirrors (iamgenesistech.com)
// do NOT accept /products/{handle} URLs and would break the "Buy Now" CTA.
const CHECKOUT_HOST = (process.env.SHOPIFY_CHECKOUT_HOST || SHOPIFY_DOMAIN || 'iamgenesistech.myshopify.com').replace(/^https?:\/\//, '').replace(/\/+$/, '');

function shopifyProductUrl(handle) {
  return `https://${CHECKOUT_HOST}/products/${handle}`;
}
...
function normalizeProduct(p) {
  ...
  return {
    ...
    productUrl: p.productUrl || shopifyProductUrl(p.handle),
    affiliateLink: p.affiliateLink || shopifyProductUrl(p.handle),
    source: 'shopify',
...
function loadLocalCache() {
  try {
    if (fs.existsSync(PRODUCT_CACHE_PATH)) {
      let raw = fs.readFileSync(PRODUCT_CACHE_PATH, 'utf8').replace(/^\uFEFF/, '');
