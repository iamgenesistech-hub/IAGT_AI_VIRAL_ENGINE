    ...
    <aside class="sidebar">
      ...
      <nav>
        ${[
          ...
          ["radar", "Pattern Library", "discovery"],
    ...
    <main class="${advancedVisible ? "" : "executive-simple-active"} section-${state.activeSection}">
      ...
      <section class="workspace-grid advanced-only">
        <div class="panel monitor">
          <div class="panel-head">
            <div>
              <h2>Commercial Pattern Library</h2>
              <p>Seed structures used by generation until live scraper and grader results are connected.</p>
            </div>
            <div class="filters">
            ...
            ${filteredAds().map((item) => `
              <button class="ad-row ${item.id === ad.id ? "selected" : ""}" data-ad="${item.id}">
                ...
                <div>
                  ...
                  <span>${item.platform} / ${item.category} / seed pattern</span>
                </div>
                <small>pattern score</small>
              </button>
            `).join("")}
          </div>
        </div>
...
function sectionDescription() {
  const descriptions = {
    ...
    discovery: "Seed commercial structures that can guide generation. This is not live scraper output until scraper and grader APIs are connected.",
  ...
  return `
    <section class="focus-layout">
      <div class="panel focus-primary">
        <div class="panel-head">
          <div><h2>Commercial Pattern Library</h2><p>Seed structures used by generation until live scraper and grader results are connected.</p></div>
          <div class="filters">${select("category", ["All", ...new Set(viralAds.map((item) => item.category))], state.category)}${select("platform", ["All", ...new Set(viralAds.map((item) => item.platform))], state.platform)}</div>
        </div>
        <div class="function-status">
          <strong>Functional role</strong>
          <span>Selects a generation pattern. It is not claiming live views, live engagement, or current social scraping yet.</span>
        </div>
        <div class="ad-list elite-list">
          ${filteredAds().map((item) => `
            <button class="ad-row ${item.id === ad.id ? "selected" : ""}" data-ad="${item.id}">
              <div class="score">${item.velocity}</div>
              <div><strong>${item.title}</strong><span>${item.platform} / ${item.category} / seed pattern</span></div>
              <small>pattern score</small>
            </button>
          `).join("")}
        </div>
...
function renderMediaCard(item, ops) {
  ...
  const hasProviderPreview = Boolean(directRenderedMediaUrl(item));
