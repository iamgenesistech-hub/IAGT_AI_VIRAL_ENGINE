/**
 * loadtest-1000.js — Load test for 1,000 concurrent users
 * 
 * Stress test variant: higher concurrency to find breaking points
 * Target: p95 latency < 10s (degraded but functional), error rate < 5%
 */
...
const RAMP_UP_SECONDS = parseInt(process.env.RAMP_UP || '45');
...
const TEST_AFFILIATES = Array.from({ length: 50 }, (_, i) => `STRESS${String(i + 1).padStart(4, '0')}`);

// Streamlined scenarios for stress testing (fewer endpoints, more realistic)
const scenarios = [
  {
    name: 'Health Check',
    weight: 0.15,
...
async function simulateUser(userId, endTime) {
  while (Date.now() < endTime) {
    ...
    // Reduced think time for stress test (5-50ms)
...
async function runLoadTest() {
  console.log(`\n╔════════════════════════════════════════════════════════════╗`);
  console.log(`║      STRESS TEST: 1,000 Concurrent Users                 ║`);
  console.log(`║      Duration: ${TEST_DURATION_SECONDS}s  |  Ramp-up: ${RAMP_UP_SECONDS}s              ║`);
  console.log(`║      Target: p95 < 10s  |  Error rate < 5%              ║`);
  console.log(`╚════════════════════════════════════════════════════════════╝\n`);
  ...
  console.log(`  p95:                 ${p95}ms ✓ TARGET (Degraded)`);
  ...
  // Pass/Fail verdict
  console.log(`\n╔════════════════════════════════════════════════════════════╗`);
  const p95Pass = p95 < 10000;
  const errorPass = parseFloat(errorRate) < 5;
  const allPass = p95Pass && errorPass;
  
  console.log(`║              ${allPass ? '✓ ACCEPTABLE' : '✗ DEGRADED'}                      ║`);
  console.log(`║  p95 latency < 10000ms: ${p95Pass ? '✓ PASS' : `✗ FAIL (${p95}ms)`}${' '.repeat(Math.max(0, 21 - `PASS (${p95}ms)`.length))}║`);
  console.log(`║  Error rate < 5%:       ${errorPass ? '✓ PASS' : `✗ FAIL (${errorRate}%)`}${' '.repeat(Math.max(0, 24 - `FAIL (${errorRate}%)`.length))}║`);
  console.log(`║  Concurrent users:      1000 ✓${' '.repeat(40)}║`);
  console.log(`╚════════════════════════════════════════════════════════════╝\n`);
