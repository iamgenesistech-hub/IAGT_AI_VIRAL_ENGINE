...
function registerMediaOutputRoutes(app, SupabaseConnector) {
  ...
  // POST /api/media-output/create-render — async: submits to HeyGen,
  ...
  // POST /api/media-output/poll-rendering
  // Sweep all rows where status='rendering', poll HeyGen for each, and
  // update rows that have finished (or failed). Safe to call periodically
  // (UI refresh, cron, or ad-hoc). Recovery path for any background poller
  // whose container was cold-shutdown by Cloud Run.
  app.post('/api/media-output/poll-rendering', async (req, res) => {
    try {
      if (!isAdminAuthorized(req)) {
        return res.status(401).json({ success: false, error: 'Admin key required (x-admin-key header).' });
      }
      if (!mediaRenderCreator || typeof mediaRenderCreator.pollPendingRenders !== 'function') {
        return res.status(503).json({ success: false, error: 'mediaRenderCreator is not available on this deployment.' });
      }
      if (!SupabaseConnector) {
        return res.status(503).json({ success: false, error: 'Supabase is not configured; cannot poll renders.' });
      }
      const result = await mediaRenderCreator.pollPendingRenders(SupabaseConnector, console);
      noStore(res);
      res.json(result);
    } catch (err) {
      console.error('[media-output/poll-rendering] failed:', err && err.stack ? err.stack : err);
